# OTT 다모아 Demo/LIVE MVP

OTT 다모아는 익명 사용자가 현재 구독 중인 OTT, 기분, 함께 보는 사람, 시청 가능 시간 등을 선택하면 안전 정책을 통과한 작품 5개를 추천하는 서비스입니다.
...
로그인, SNS/OAuth, Profile, MY, 찜, 봤어요, 서버 저장 관심 없음은 v0.8 LIVE 범위에 포함되지 않습니다.
...
| `/` | 서비스 소개와 익명 추천 진입 |
| `/choice` | 익명 CHOICE 입력 |
| `/recommendations/[runId]` | TOP1과 후보 4개, Trace, 승인·교체 |
...
| `POST` | `/api/recommendations/[runId]/approval` | runtime 완화 승인 또는 거절 |
...
익명 Run 목록 API는 제공하지 않습니다. LIVE health는 비용을 발생시키지 않도록 catalog·Run·Trace 저장소만 실제 조회하고, OpenAI search·selector는 `not_checked`로 표시합니다. 공개 오류는 정제된 코드만 반환하며 stack, DB/provider 원문, prompt, secret을 노출하지 않아야 합니다.
...
v0.8 병렬 작업은 다음 단일 작성자 경계를 유지합니다.
...
`TEAM-LLM-START.md`는 이후 별도 검토·커밋할 팀 인수인계 문서입니다. 현재 LIVE 구현 변경에는 포함하지 않으며 이 README 작업에서도 수정하지 않습니다.
