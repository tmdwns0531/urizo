...
Run → 공개 Trace 흐름이다. 추천 경로 자체는 계정 없이 동작하며 계정 표를 읽지
않는다.

계정과 찜은 PR #16에서 제품 범위에 들어왔다. 별명·비밀번호 로그인, 계정에 저장하는
찜, 찜한 작품의 OTT 커버리지 계산이 활성이다. Profile, MY, 봤어요, 서버 저장 관심
없음은 여전히 제품 범위가 아니다. 관람등급 판정(`getMvpFilterReasons`)은 계정과
무관하게 남으며 로그인해도 완화되지 않는다.
