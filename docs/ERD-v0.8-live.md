...
`AWAITING_APPROVAL`/`COMPLETED`에는 세 필드와 status에 맞는 response JSON이 필수이고,
`FAILED`는 response를 두지 않고 allowlist `errorCode`와 `completedAt`을 가진다. DB lifecycle
CHECK가 이 상태 shape을 강제한다. Run CAS와 해당 Trace batch는 하나의 transaction이다.
