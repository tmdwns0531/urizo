...
export function RecommendationShowcase() {
  return (
    <section
      ...
      className="cinema-showcase relative scroll-mt-20 overflow-hidden border-b border-white/10 bg-[#121820] py-20 sm:py-28"
        ...
        <div className="mt-12 grid overflow-hidden rounded-[1.75rem] border border-white/10 bg-[#0e141b] shadow-[0_30px_90px_rgba(0,0,0,.28)] sm:mt-16 lg:grid-cols-[.75fr_1.25fr]">
          <div className="border-b border-white/10 p-6 sm:p-9 lg:border-r lg:border-b-0 lg:p-10">
            <div className="flex items-center justify-between gap-4">
              <span className="rounded-full border border-[#ff7043]/25 bg-[#ff7043]/10 px-3 py-2 text-sm font-bold text-[#ffad93]">예시 화면</span>
              <span className="text-sm text-slate-400">조건 확인 완료</span>
            </div>
            <p className="mt-8 text-sm font-bold tracking-[0.12em] text-slate-400">오늘의 상황</p>
            ...
            <ul className="mt-6 flex flex-wrap gap-2" aria-label="적용 조건 예시">
              {["혼자", "편안한", "2시간 안에", "이용 OTT만"].map((condition) => (
                <li key={condition} className="rounded-full border border-white/10 bg-white/5 px-3 py-2 text-sm font-medium text-slate-200">
                  {condition}
                </li>
              ))}
            </ul>
            <div className="mt-8 border-t border-white/10 pt-6">
              <p className="flex gap-2 text-base leading-7 text-slate-300">
            ...
            {PREVIEW_POSTERS.map((poster, index) => (
              <article key={poster.id} className="w-[min(78vw,17rem)] shrink-0 snap-center rounded-xl border border-white/10 bg-[#17202a] p-2.5 sm:w-auto">
                ...
                <div className="px-1 pb-1 pt-3">
                  <p className="text-sm font-bold tracking-[0.1em] text-[#ff9f82]">추천 {index + 1}</p>
                  <h4 className="mt-1 truncate text-sm font-extrabold text-white sm:text-base">{poster.title}</h4>
                  <p className="mt-2 text-sm leading-6 text-slate-300">{poster.year} · {poster.detail}</p>
                  <p className="mt-2 truncate text-sm font-medium text-slate-300">{poster.provider}</p>
                </div>
              </article>
            ))}
          </div>
