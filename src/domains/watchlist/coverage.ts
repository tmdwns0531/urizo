...
/**
 * 찜한 작품 하나. 저장 형태가 아니라 커버리지 계산에 필요한 만큼만 담은
 * 화면용 표현이다.
 *
 * 찜 표에는 계정 id 와 작품 id 만 남기고, 조회할 때 카탈로그에서 채워
 * 내려준다 (`app/api/watchlist/route.ts`). 제목·포스터를 찜 표에 복사해 두면
 * 작품 정보가 바뀌어도 옛 값이 남기 때문이다.
 *
 * `providers` 를 함께 담는 이유는 커버리지 계산에 필요한 것이 그 목록뿐이라,
 * 한 번 받아두면 화면에서 서버 왕복 없이 바로 계산할 수 있어서다.
 */
